...
int main(int argc, char **argv) {
    ...
    std::string dpath(argv[1]);//("/gpfs/alpine/proj-shared/csc143/gongq/andes/TC-CaseStudy/mgard-test/stb_layout/");
    std::vector<std::string> fname;
    struct dirent *de;
    DIR *dr = opendir(dpath.c_str());
    if (dr == NULL) {
        std::cout << "Could not open the directory: " << dpath << "\n";
    }
    while ((de = readdir(dr)) != NULL) {
        fname.push_back(de->d_name);
    }
    std::cout << "total readin " << fname.size() -2 << " file\n";
...
//    for (size_t data_id=0; data_id<data_f.size(); data_id++) {
    for (size_t data_id=2; data_id<fname.size(); data_id ++) {
//        for (size_t suff_id=0; suff_id<suffx.size(); suff_id++) {
            if (rank==0)
                std::cout << "readin: " << fname.at(data_id) << "\n"; 
            adios2::IO reader_io = ad.DeclareIO("Input"+std::to_string(data_id));
            adios2::IO writer_io = ad.DeclareIO("Output"+std::to_string(data_id));
//            adios2::Engine reader = reader_io.Open(dpath+fname+data_f[data_id]+"-21600.tcv5"+suffx[suff_id], adios2::Mode::Read);
            adios2::Engine reader = reader_io.Open(dpath+fname.at(data_id), adios2::Mode::Read);
            adios2::Engine writer = writer_io.Open("./2D/1e-4/5yr/"+fname.at(data_id) + ".mgard", adios2::Mode::Write);
...
//                    std::cout << "rank " << rank << " read " << var_name[ivar] << " in " << suffx[suff_id] << " step " << r_step << "\n";
                    const mgard::CompressedDataset<2, float> compressed = mgard::compress(hierarchy, var_in.data(), (float)0.0, tol);
